---
title: "1.2 Transformer 的标准流程"
url: /notes/transformer-agent/13-transformer/02/
type: notes
math: true
weight: 1302
chapter: "13"
...
设词表大小为 $V$，隐藏维度为 $d$。**嵌入矩阵**（Embedding Matrix）为
$$
...
![Embedding](/notes/transformer-agent/assets/TransEmbedding.png)
...
> **Example**
>
> 在我们的翻译句子中，10个位置分别获得不同的位置编码。设 $d=8$，前两个频率维度： > - $i=0$: $\omega_0 = 1$，波长 $= 2\pi$（高频，区分相邻 token） > - $i=1$: $\omega_1 = 10000^{-2/8} = 10000^{-0.25} \approx 0.1$，波长 $\approx 62.8$（低频，区分大段位置）
>
>
...
$$
>
> 在句子`The cat sat on the mat because it was tired`,
> 中， ``cat'' 位于位置 $p=2$，而 ``it'' 位于位置 $p=8$。二者的相对距离为 $\Delta = 6$, 高频分量可以帮助模型区分相邻的 token，例如 ``cat'' 和 ``sat''；低频分量则可以让模型感知 ``it'' 与 ``cat'' 之间相隔了 6 个位置。具体地，在低频分量 $i=1$ 上，二者产生的相位差为
$$
...
![Self-Attention](/notes/transformer-agent/assets/TransSA.png)

### 1.2.4.1 Query、Key、Value 的严格定义
...
- $q_t$（Query，第 $t$ 行）："我在寻找什么信息？" - $k_t$（Key，第 $t$ 行）："我有什么标签/索引？" - $v_t$（Value，第 $t$ 行）："如果被关注，我提供什么内容？"

### 1.2.4.2 缩放点积注意力的完整推导
...
$$
> 因此，自注意力本质上是在做一种**数据依赖的条件平均**：根据当前 query 的内容，动态决定从哪些位置"读取"信息以及读多少。
...
> **为什么要除以 $\sqrt{d_k}$：完整的概率论推导**
>
> 这是 Transformer 原论文最著名但常被轻描淡写的细节。我们给出完整的逐步推导。
>
> **假设条件：** Query 向量 $q$ 和 Key 向量 $k$ 的各分量相互独立，各分量都经过适当初始化，使得
$$
...
>
> **点积的均值：**考虑到独立性：$\mathbb{E}[q_\alpha k_\alpha] = \mathbb{E}[q_\alpha]\mathbb{E}[k_\alpha] = 0 \cdot 0 = 0$
$$
...
$$
>
> **点积的方差：** 首先计算每一项 $q_\alpha k_\alpha$ 的方差。因为均值为0，
$$
...
$$
>
> **结论：** 点积 $q\cdot k$ 的标准差为 $\sqrt{d_k}$。若 $d_k=64$，点积的典型值为 $0 \pm 8$ 左右；若 $d_k=1024$，典型值就是 $0 \pm 32$。不做缩放的话，softmax 的输入绝对值会随 $d_k$ 增大而增大，导致 softmax 输出趋近于 one-hot（最大值位置概率接近1，其余接近0）。
>
> 当 softmax 输出接近 one-hot 时，其雅可比矩阵接近零矩阵（因为方差接近0），梯度几乎为零——这就是**梯度消失**。除以 $\sqrt{d_k}$ 将方差拉回到 $O(1)$：
$$
...
### 1.2.4.3 因果掩码：自回归生成中的 Self-Attention 约束

自回归模型预测 $x_t$ 时只能使用 $x_{\lt t}$。在 Transformer 中通过**因果掩码**（Causal Mask）实现。定义掩码矩阵 $M\in\mathbb{R}^{n\times n}$：
$$
\begin{aligned}
M_{ij} = \begin{cases}
0, & j \le i,\\
...
$$
\text{head}^{(r)} = \operatorname{Attn}\!\left(HW_Q^{(r)},\, HW_K^{(r)},\, HW_V^{(r)}\right),
$$
...
> **Example**
>
> 在句子`The cat sat on the mat because it was tired`， ``cat'' 在位置 2，``it'' 在位置 8。
>
...
## 1.2.6 FFN / Residual / LayerNorm

每个 Transformer 层除了注意力还包含**前馈网络**（Feed-Forward Network, FFN），也叫**多层感知机**（Multi-Layer Perceptron，MLP）：
$$
...
![Multi-Layer Perception](/notes/transformer-agent/assets/TransMLP.png)

**残差连接**和**层归一化**（Residual和Layer Normalization）确保深层网络可训练。
...
> **Example**
>
> 考虑翻译句子`The cat sat on the mat because it was tired`，共10个token。设某一层 Transformer 的输入隐状态为 $H\in\mathbb{R}^{10\times d}$，其中第 $t$ 行 $H_t\in\mathbb{R}^d$ 表示第 $t$ 个 token 的当前表示。
>
...
$$
> 因此，经过一个完整的 Transformer 层后，`it'' 的表示不仅保留了自身作为代词的信息，还融合了 `cat'' 的指代信息、`because'' 的因果结构信息以及 `tired'' 的状态信息。这使得模型在翻译时更可能生成`这只猫坐在垫子上，因为它累了。`
> 而不是错误地把 `it'' 理解为 `mat''。
>
...
## 1.2.7 Probability：从隐藏状态到下一个 token
...
\quad \tau>0.
$$
其中 $\tau$ 是温度参数。若 $\tau\lt 1$，则不同 logit 之间的差距被放大，概率分布更尖锐，模型更倾向于选择高分 token；若 $\tau>1$，则 logit 差距被压缩，概率分布更平坦，低分 token 也有更多机会被采样。
...
> **Example**
>
> 翻译例子：```The cat sat on the mat because it was tired.''`
>
> 目标输出可以是：```猫坐在垫子上，因为它累了。''`
>
...
$$
>
> 如果使用温度采样，当 $\tau\lt 1$ 时，``它'' 的优势会被进一步放大，模型几乎一定生成 ``它''；当 $\tau>1$ 时，``猫'' 或其他候选 token 的概率会相对上升，生成结果会更随机。
>
...
## 1.2.8 Encoder 与 Decoder：原始 Transformer 的两种模块

前面介绍的 tokenization、embedding、位置编码、self-attention、multi-head attention、FFN、残差连接和 LayerNorm，是原本 Transformer 的基本组成部分，可以分成两类：
- **Encoder**：负责把输入序列编码成上下文化表示；
- **Decoder**：负责在已有输出前缀的条件下逐步生成目标序列。

设源序列为 $x=(x_1,\dots,x_n)$，目标序列为 $y=(y_1,\dots,y_m)$。 Transformer 建模的是条件生成问题：
$$
P(y|x) = \prod_{t=1}^{m} P(y_t \mid y_{\lt t},x).
$$
其中，Encoder 负责处理完整输入 $x$，Decoder 负责根据已经生成的目标前缀 $y_{\lt t}$ 和 Encoder 输出，预测下一个目标 token $y_t$。

### 1.2.8.1 Encoder：把输入序列编码成上下文表示
...
- **双向 self-attention**：输入序列中的每个 token 都可以关注同一序列中的所有 token；
- **FFN、残差连接和 LayerNorm**：对每个位置的表示做非线性变换并稳定训练。
...
### 1.2.8.2 Decoder：在条件上下文下逐步生成输出

Decoder 的输入是目标序列的前缀。生成第 $t$ 个 token 时，Decoder 只能看到已经生成的 $y_{\lt t}$，不能看到未来的 $y_t,y_{t+1},\dots$。
...
Decoder 的每一层包含三部分：
- **因果 self-attention**：只允许当前位置关注过去位置；
- **cross-attention**：用 Decoder 当前表示作为 Query，读取 Encoder 输出 $Z$；
- **FFN、残差连接和 LayerNorm**：进一步加工每个位置的表示。
...
$$
P(y_t \mid y_{\lt t},x) = p_t[y_t].
$$

> **Cross-Attention 的具体含义**
>
...
$$
>
> 这说明，Decoder 在生成目标 token 时，并不是只看目标前缀 $y_{\lt t}$，还会通过 cross-attention 动态读取源序列表示 $Z$。
>
> 因此，Decoder 有两种注意力：
> - 因果 self-attention 负责“看已经生成的目标前缀”；
> - cross-attention 负责“看 Encoder 对源序列的编码结果”。

> **Example**
>
> 在本文的翻译例子中，源句为："`The cat sat on the mat because it was tired.`"
> 目标句可以是：`“猫坐在垫子上，因为它累了。”`
>
> Encoder 读取完整英文句子后，得到上下文化表示 $Z$。当 Decoder 生成中文中的“它”时：
> - 因果 self-attention 只能看到已经生成的中文前缀，例如“猫 坐 在 垫子 上 ， 因为”；
> - cross-attention 可以读取英文源句的编码结果 $Z$，从而关注英文中的 ``it''、``cat'' 等位置。
>
>
...
> - Encoder 负责对输入序列 $x$ 做双向上下文编码，得到 $Z$； > - Decoder 负责根据目标前缀 $y_{\lt t}$ 和 Encoder 输出 $Z$，逐步生成目标 token； > - 原始 Transformer 建模的是条件概率 $P(y|x)$。
>
>
...
## 1.2.9 Encoder-only、Decoder-only 与 Encoder-Decoder 架构

在原始 Transformer 中，Encoder 和 Decoder 同时存在。但在后续发展中，人们发现可以根据任务类型保留其中一部分结构，于是形成了三类常见架构：
- **Encoder-only**：只使用 Encoder，适合理解和表示学习任务；
- **Decoder-only**：只使用带因果掩码的 Decoder，适合自回归生成任务；
- **Encoder-Decoder**：同时使用 Encoder 和 Decoder，适合条件生成任务。

三者共享 Transformer block 的基本思想，但由于可见信息不同、训练目标不同，最终模型性质也不同。

### 1.2.9.1 Encoder-only：面向表示学习的架构
...
由于 Encoder 的 self-attention 是双向的，因此每个位置都可以看到整个输入序列。
这类模型更适合做理解型任务，例如：文本分类；命名实体识别；句子匹配；语义检索；表示学习。
...
### 1.2.9.2 Decoder-only：面向自回归生成的架构

Decoder-only 架构保留带因果掩码的 self-attention，不再显式区分源序列和目标序列。
它直接对单个 token 序列建模：
$$
P(x_1,x_2,\dots,x_n) = \prod_{t=1}^{n} P(x_t \mid x_{\lt t}).
$$

这正是ALM的概率分解方式。
由于每一步都只依赖前文，因此 Decoder-only 模型可以自然地用于文本生成：给定 prompt；预测下一个 token；把生成的 token 接回上下文；继续预测下一个 token。

现代通用 LLM 大多采用 Decoder-only 架构，例如 GPT 类模型和 LLaMA 类模型。

### 1.2.9.3 Encoder-Decoder：面向条件生成的架构

Encoder-Decoder 架构同时保留 Encoder 和 Decoder。
它不是直接建模单个序列的联合概率，而是建模输入序列到输出序列的条件概率：
$$
P(y|x) = \prod_{t=1}^{m} P(y_t \mid y_{\lt t},x).
$$
...
> - **Encoder-only**：每个位置可以看到整个输入序列，所以适合理解；
> - **Decoder-only**：每个位置只能看到前文，所以适合从左到右生成；
> - **Encoder-Decoder**：Encoder 可以看完整输入，Decoder 只能看目标前缀，但可以通过 cross-attention 读取 Encoder 输出。
>
> 因此，三者对应三种不同的条件结构：
$$
\begin{aligned}
\text{Encoder-only:}&\quad P(x_i \mid x_{\backslash \mathcal{M}})\\
\text{Decoder-only:}&\quad P(x_t \mid x_{\lt t}),\\
\text{Encoder-Decoder:}&\quad P(y_t \mid y_{\lt t},x).
\end{aligned}
$$
...
> - **Encoder-only：理解任务**
>
>     输入句子：`The cat was tired.`
>
>     任务是判断句子情感、识别实体或生成句向量。此时模型更关心整句表示 $H^{(L)}$，而不是逐步生成下一个 token。
> - **Decoder-only：续写任务**
>
>     输入前缀：`The cat sat on the mat because`
>
>     模型需要预测下一个 token：$P(x_t \mid x_{\lt t}).$ 它可能继续生成 ``it was tired''。这就是 GPT 类 LLM 的基本生成方式。
> - **Encoder-Decoder：翻译任务**
>
>     输入完整英文句子：`The cat sat on the mat because it was tired.`
>
>     输出中文句子：`“猫坐在垫子上，因为它累了。”`此时模型建模的是：
$$
P(y|x) = \prod_{t=1}^{m} P(y_t \mid y_{\lt t},x).
$$
>
...
| **架构** | **可见信息** | **主要目标** | **代表模型** |
| --- | --- | --- | --- |
| Encoder-only | 整个输入序列，双向可见 | 表示学习、理解任务 | BERT |
| Decoder-only | 只能看见前文，单向生成 | 自回归生成 $P(x_t|x_{\lt t})$ | GPT, LLaMA | | Encoder-Decoder | 输入全可见，输出端单向生成 | 条件生成 $P(y_t|y_{\lt t},x)$ | Transformer, T5, BART |

> **Note**
>
> Transformer 的三类典型组织方式如下：
> - **Encoder-only**：只保留 Encoder，适合理解和表示学习；
> - **Decoder-only**：只保留因果 Decoder，适合自回归文本生成，现代通用 LLM 的主流结构；
> - **Encoder-Decoder**：同时使用 Encoder 和 Decoder，适合翻译、摘要等条件生成任务。
>
>
> 因此，encoding 和 decoding 可以分成两层理解：
> - 在输入处理层面，tokenization、embedding 和位置编码属于输入 encoding；
> - 在模型架构层面，Encoder 负责上下文化表示，Decoder 负责条件生成或自回归生成。
>
>

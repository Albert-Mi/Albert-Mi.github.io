...
自回归模型预测 $x_t$ 时只能使用 $x_{<t}$。在 Transformer 中通过**因果掩码**（Causal Mask）实现。定义掩码矩阵 $M\in\mathbb{R}^{n\times n}$：
$$
\begin{aligned}
M_{ij} = \begin{cases}
0, & j \le i,\\
...
> - Encoder 负责对输入序列 $x$ 做双向上下文编码，得到 $Z$； > - Decoder 负责根据目标前缀 $y_{<t}$ 和 Encoder 输出 $Z$，逐步生成目标 token； > - 原始 Transformer 建模的是条件概率 $P(y|x)$。
>
>
...
| Decoder-only | 只能看见前文，单向生成 | 自回归生成 $P(x_t|x_{<t})$ | GPT, LLaMA | | Encoder-Decoder | 输入全可见，输出端单向生成 | 条件生成 $P(y_t|y_{<t},x)$ | Transformer, T5, BART |
